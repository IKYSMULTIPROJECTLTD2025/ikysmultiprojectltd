...
import { loginUser } from "../utils/account/auth/login";
...
const Sign_In = () => {
  ...
  const handleSignIn = async () => {
    try {
      ...
      if (loginResponse.success) {
          ...
          router.push('/Sign_In/verify');
       
      } else  {
       
        setLoginError(`Error logging in: ${loginResponse.error}`);
      }
    } catch (error:any) {
  ...
  return (
    <div className='py-10 pt-15 font-bold  h-screen bg-lime-500'>
      <div>
      ...
      <form >
        ...
        <div className='flex justify-center mt-3 '>
          ...
          <label htmlFor='agreeTerms' className='text-xs text-indigo-950'>
       ...
       do not have an account? <span className='text-indigo-950 text-lg font-bold
        '>Sign Up</span></p> </Link>
      </div>

      <div className='flex justify-center '>
       <Link href={{pathname: '/Sign_In/Forgot_Password'}}> <p className='text-sm  mt-5'>
       <span className='text-indigo-950 text-sm font-bold
