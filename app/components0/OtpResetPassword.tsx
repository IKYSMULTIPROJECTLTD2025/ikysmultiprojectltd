...
import OTPResetPasswordUtils from '../utils/account/auth/verifyResetPassword';
...
const OtpResetPassword = () => {
  ...
  const handleResend = async () => {
    try {
      ...
      setSuccessMessage('');
      setErrorMessage('');
      await OTPResetPasswordUtils.resendOTP(email);
  ...
  const handleVerify = async () => {
    try {
      ...
      const verifyResponse = await OTPResetPasswordUtils.verifyUserOtp({
        email,
        otp,
      });
      ...
      if (verifyResponse.message === 'OTP verified successfully') {
        ...
        console.error('Error verifying OTP:', verifyResponse.error);
        setErrorMessage(`Error verifying OTP: ${verifyResponse.error}`);
        setSuccessMessage('');
      }
    } catch (error: any) {
  ...
  return (
    <div className='h-screen  bg-lime-500 font-bold'>
      <div>
        <h1 className='flex justify-center text-lg text-slate-950 pt-20 max-md:text-sm'>
