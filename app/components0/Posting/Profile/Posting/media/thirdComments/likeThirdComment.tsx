...
import { likeThirdCommentsImages, getLikeThirdCommentsImages } from '@/app/utils/likeImages';
...
const LikeThirdComment: React.FC<LikeButtonProps> = ({ thirdCommentId }) => {
  ...
  useEffect(() => {
    ...
    const fetchLikeCount = async () => {
      ...
      try {
        setLoading(true);
        const count = await getLikeThirdCommentsImages(thirdCommentId);
        setLikeCount(count);
        setLoading(false);
      } catch (error) {
  ...
  const handleLikeClick = async () => {
    try {
      ...
      setLoading(true);
          await likeThirdCommentsImages(thirdCommentId);
