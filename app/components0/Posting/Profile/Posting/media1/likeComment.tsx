...
import { likeCommentsBanner, getLikeCommentsBanner } from '@/app/utils/likeBanner';
...
const LikeComment: React.FC<LikeButtonProps> = ({ commentId }) => {
  ...
  useEffect(() => {
    ...
    const fetchLikeCount = async () => {
      ...
      try {
        setLoading(true);
        const count = await getLikeCommentsBanner(commentId);
        setLikeCount(count);
        setLoading(false);
      } catch (error) {
  ...
  const handleLikeClick = async () => {
    try {
      ...
      setLoading(true);
          await likeCommentsBanner(commentId);
