...
const VideosDetails: React.FC = () => {
  ...
  return (
   ...
   <Image
     src="/loading-gift.gif"
     alt="loading-gift"
     width={500}
     height={500}
     className="border-2 border-black rounded-sm object-cover w-full h-full"
   />
    <div className="absolute inset-0 bg-black bg-opacity-10 rounded-sm"></div>
        ...
        <div className='grid grid-cols-2 max-xl:grid-cols-1 gap-1'>
          ...
          <span className='  bg-slate-200   border-2 w-full border-slate-950 px-5 py-2'>

            <p className='text-xl max-md:text-lg  max-sm:text-sm mb-2'>see full product details below</p>
      ...
      <span className='flex justify-between mb-1 text-lg  max-sm:text-sm'>
      ...
      <p className=' mb-2 text-lg  max-sm:text-sm'>Company Name: {productVideos[0].companyName ? productVideos[0].companyName : " was not provided"}</p>

      <p className="mb-2 text-lg max-sm:text-sm overflow-hidden">Company Link:{" "}
      {productVideos[0]?.companyLink ? ( <a href={ productVideos[0].companyLink.startsWith("http://") ||
      productVideos[0].companyLink.startsWith("https://")  ? productVideos[0].companyLink
      : `https://${productVideos[0].companyLink}` } target="_blank" rel="noopener noreferrer" 
       className="text-blue-700 ml-5"> {productVideos[0].companyLink}</a>) : ( "was not provided")}</p>
       ...
       <button className='w-full h-8 text-white mt-1 text-sm  max-sm:text-sm rounded-sm bg-slate-950'>
          <h1>FULL DESCRIPTION </h1>
       </button>

       <span className="flex flex-col m-1 text-sm max-sm:text-sm">
           <p className="text-sm  max-sm:text-sm">DESCRIPTION OF THE PRODUCT:</p>
            <p>
             {productVideos && productVideos[0]?.description
             ? isExpanded
             ? productVideos[0].description
             : productVideos[0].description.length > 100
             ? productVideos[0].description.slice(0, 100) + "..."
             : productVideos[0].description
