...
import { getProductByIdShare } from '@/app/utils/images';
...
const ShareProductDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductImages = async () => {
      ...
      try {
        ...
        //@ts-ignore
        const response: ImageResponse = await getProductByIdShare(id);
  ...
  return (
   ...
   />
    <div className="absolute inset-0 bg-black bg-opacity-10 rounded-sm"></div>
      ...
      {productImages[0]?.companyLink ? ( <a href={ productImages[0].companyLink.startsWith("http://") ||
      productImages[0].companyLink.startsWith("https://")  ? productImages[0].companyLink
      : `https://${productImages[0].companyLink}` } target="_blank" rel="noopener noreferrer" 
       className="text-blue-700 ml-5"> {productImages[0].companyLink}</a>) : ( "was not provided")}</p>
       ...
       <span className="flex flex-col m-1 text-sm max-sm:text-sm">
           <p className="text-sm  max-sm:text-sm">DESCRIPTION OF THE PRODUCT:</p>
            <p>
             {productImages && productImages[0]?.description
             ? isExpanded
             ? productImages[0].description
             : productImages[0].description.length > 100
             ? productImages[0].description.slice(0, 100) + "..."
             : productImages[0].description
...
export default ShareProductDetails;
