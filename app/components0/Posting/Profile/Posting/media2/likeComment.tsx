...
import { likeCommentsVideo, getLikeCommentsVideos } from '@/app/utils/likeVideo';
...
const LikeComment: React.FC<LikeButtonProps> = ({ commentId }) => {
  ...
  useEffect(() => {
    ...
    const fetchLikeCount = async () => {
      ...
      try {
        setLoading(true);
        const count = await getLikeCommentsVideos(commentId);
        setLikeCount(count);
        setLoading(false);
      } catch (error) {
  ...
  const handleLikeClick = async () => {
    try {
      ...
      setLoading(true);
          await likeCommentsVideo(commentId);
