...
import { getBannerByIdShare } from '@/app/utils/banner';
...
const ShareBannerDetails: React.FC = () => {
  ...
  const handleCommentClick = ()=>{
    setCommentVisible(!commentVisible);
      ...
      const extractIdFromPathname = (pathname: string) => {
        const match = pathname?.match(/\/images\/(.+)$/); // Match everything after "/images/"
        return match ? match[1] : null; // If matched, return the ID
      };
    
      const id = extractIdFromPathname(pathname); // Extract ID
  ...
  useEffect(() => {
    const fetchProductBanners = async () => {
      ...
      try {
        //@ts-ignore
        const response: ImageResponse = await getBannerByIdShare(id);
  ...
  return (
   ...
   />
    <div className="absolute inset-0 bg-black bg-opacity-10 rounded-sm"></div>
      ...
      <p className="mb-5 text-lg max-sm:text-sm overflow-hidden">Company Link:{" "}
      {productBanners[0]?.companyLink ? ( <a href={ productBanners[0].companyLink.startsWith("http://") ||
      productBanners[0].companyLink.startsWith("https://")  ? productBanners[0].companyLink
      : `https://${productBanners[0].companyLink}` } target="_blank" rel="noopener noreferrer" 
       className="text-blue-700 ml-5"> {productBanners[0].companyLink}</a>) : ( "was not provided")}</p>
       ...
       <span className="flex flex-col m-1 text-sm max-sm:text-sm">
           <p className="text-sm  max-sm:text-sm">DESCRIPTION OF THE PRODUCT:</p>
            <p>
             {productBanners && productBanners[0]?.description
             ? isExpanded
             ? productBanners[0].description
             : productBanners[0].description.length > 100
             ? productBanners[0].description.slice(0, 100) + "..."
             : productBanners[0].description
...
export default ShareBannerDetails;
