...
import { likeThirdCommentsBanner, getLikeThirdCommentsBanner } from '@/app/utils/likeBanner';
...
const LikeThirdComment: React.FC<LikeButtonProps> = ({ thirdCommentId }) => {
  ...
  useEffect(() => {
    ...
    const fetchLikeCount = async () => {
      ...
      try {
        setLoading(true);
        const count = await getLikeThirdCommentsBanner(thirdCommentId);
        setLikeCount(count);
        setLoading(false);
      } catch (error) {
  ...
  const handleLikeClick = async () => {
    try {
      ...
      setLoading(true);
          await likeThirdCommentsBanner(thirdCommentId);
