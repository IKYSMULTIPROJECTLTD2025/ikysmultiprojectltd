...
const ProductImages: React.FC = () => {
  ...
  return (
    <div className='font-bold w-full text-sm min-h-screen overflow-hidden'>
      {isLoading ? (
        ...
        <>
          {productsWithImages.length > 0 ? (
              ...
              {productsWithImages.map((product) => (
                ...
                className='border-2 border-slate-950 bg-white p-2'>
                  <span className='flex justify-between mb-2 text-sm'>
                    ...
                    <SharingImages 
                     productUrl={`http://localhost:3000/Product/share/images/${product.id}`}
                     //@ts-ignore
                  ...
                  <a href={ product.companyLink.startsWith("http://") ||
                  product.companyLink.startsWith("https://")  ? product.companyLink
                  : `https://${product.companyLink}` } target="_blank" 
                  rel="noopener noreferrer" 
                  className="overflow-hidden text-xs text-blue-800"> 
                  {product.companyLink}</a>) : ( "was not provided")}</p>
            ...
            max-md:text-lg text-blue-950'>Be The First To Upload Products Here!</p>
          )}
        </>
      )}
    </div>
